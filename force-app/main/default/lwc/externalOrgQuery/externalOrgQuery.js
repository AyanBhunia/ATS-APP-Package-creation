...
export default class ExternalOrgQuery extends LightningElement {
    ...
    @track importStatus = {
        ...
        detailedMessages: [],
        report: {
            success: [],  // Array of {object, count, idMap: Map<sourceId, destinationId>}
            error: []     // Array of {object, count, errorMap: Map<sourceId, errorMsg>}
        }
    };
    ...
    // Matching state
    idRemap = new Map();                  // Map<sourceId, destinationId>
    matchingQueryList = [];               // reversed copy of finalExportQueries for iteration
    @track showMatchingUI = false;        // controls matching UI visibility
    @track currentMatchingObject;         // current query def being matched {objectName, soql, fields, count}
    @track matchingFieldsOptions = [];    // [{label, value}]
    @track selectedMatchingFields = [];   // ['Field__c', ...]

    // Separate outputs for Check & Import vs Import All
    @track matchingLogs = [];             // array of strings for "check & import" only
    @track importLogs = [];               // array of strings for "import all" only (kept in parallel to importStatus)

    // Check & Import wizard state
    @track showMatchingUIWizard = false;
    @track matchingStep = 'select'; // 'select' | 'results' | 'report'
    // Render-friendly snapshots for template (because LWC template cannot call functions)
    @track matchingObjects = []; // ordered list of object API names
    @track matchingFieldOptionsByObject = {}; // { [objectName]: [{label,value}] }
    @track selectedMatchingFieldsByObject = {}; // { [objectName]: string[] }
    @track matchingCounts = {}; // { [objectName]: { sourceCount, destCount, matchedCount, unmatchedCount } }
    @track importReport = []; // [{ objectName, successCount, errorCount }]

    // Internal maps not directly rendered
    matchingConfigByObject = new Map(); // objectName -> string[]
    matchingCountsByObject = new Map(); // objectName -> { sourceCount, destCount, matchedCount, unmatchedIds: string[] }
    matchedIdMap = new Map(); // objectName -> Map<sourceId, destinationId>
    finalIdRemap = new Map(); // objectName -> Map<oldId, newId>
    ...
    handleSoqlChange = (event) => { this.soql = event.target.value; this.clearAfter('soql'); };
    handleObjectChange = (event) => { this.selectedObject = event.detail.value; this.dependencyTree = undefined; this.clearAfter('object'); };
    
    // Destination input handlers
    handleDestUsernameChange = (event) => { this.destUsername = event.target.value; this.destTestMessage = undefined; this.clearAfter('destCredentials'); };
    handleDestPasswordChange = (event) => { this.destPassword = event.target.value; this.destTestMessage = undefined; this.clearAfter('destCredentials'); };
    handleDestEnvironmentChange = (event) => { this.destEnvironment = event.detail.value; this.destTestMessage = undefined; this.clearAfter('destCredentials'); };
    handleDepthChange = (event) => {
        const raw = parseInt(event.target.value, 10);
        const safe = Number.isFinite(raw) && raw >= 0 ? raw : 0;
        this.maxDepth = safe;
        this.dependencyTree = undefined;
        this.clearAfter('depth');
    };
    ...
    handleExcludedObjectsChange = (event) => { this.excludedObjects = event.detail.value || []; this.dependencyTree = undefined; this.clearAfter('excludedObjects'); };
    handleSelectStandardObjects = () => { this.excludedObjects = this.standardObjects; this.dependencyTree = undefined; this.clearAfter('excludedObjects'); };
    handleSelectCustomObjects = () => { this.excludedObjects = this.customObjects; this.dependencyTree = undefined; this.clearAfter('excludedObjects'); };
    handleClearExclusions = () => { this.excludedObjects = []; this.dependencyTree = undefined; this.clearAfter('excludedObjects'); };
    ...
    get importProgressPercentage() {
        if (!this.importStatus.totalObjects) return 0;
        return Math.round((this.importStatus.processedObjects / this.importStatus.totalObjects) * 100);
    }
    ...
    get hasImportResults() {
        return this.importStatus?.detailedMessages?.length > 0;
    }

    get hasSuccessReport() {
        return this.importStatus?.report?.success?.length > 0;
    }

    get hasErrorReport() {
        return this.importStatus?.report?.error?.length > 0;
    }

    get formattedSuccessReport() {
        if (!this.hasSuccessReport) return '';
        try {
            return JSON.stringify(this.importStatus?.report?.success || [], null, 2);
        } catch (e) {
            console.error('Error formatting success report:', e);
            return '';
        }
    }

    get formattedErrorReport() {
        if (!this.hasErrorReport) return '';
        try {
            return JSON.stringify(this.importStatus?.report?.error || [], null, 2);
        } catch (e) {
            console.error('Error formatting error report:', e);
            return '';
        }
    }
    ...
    async handleFinalExport() {
        // When building fresh final queries, clear prior matching/import outputs
        this.matchingLogs = [];
        this.importLogs = [];
        if (this.importStatus) {
            this.importStatus.detailedMessages = [];
            this.importStatus.successCount = 0;
            this.importStatus.errorCount = 0;
        }
    ...
    // Legacy entry remains; keep behavior unchanged
    handleCheckMatchingDestination = () => {
        // Entry point for legacy "matching UI" (kept for backward compatibility)
        this.error = undefined;
        ...
        if (!this.isDestinationCurrentOrg && !(this.destSessionId && this.destInstanceUrl)) {
            this.error = 'Please test Destination connection first.';
            return;
        }

        // Clear matching-only output area (do not mix with import)
        this.matchingLogs = [];

        // Reset matching state
        this.idRemap = new Map();
        this.matchingQueryList = Array.isArray(this.finalExportQueries) ? [...this.finalExportQueries].reverse() : [];
        this.showMatchingUI = false;
        this.currentMatchingObject = undefined;
        this.matchingFieldsOptions = [];
        this.selectedMatchingFields = [];

        // Start iterative UI
        this.displayNextMatchingCheck();
    };

    // New Wizard: open
    handleOpenCheckImport = () => {
        this.error = undefined;
        if (!this.hasFinalQueries) {
            this.error = 'No final export queries available. Run Final Export first.';
            return;
        ...
        // Build bottom-to-top object list from finalExportQueries
        const order = Array.from(new Set(this.finalExportQueries.map(q => q.objectName))).reverse();
        this.matchingObjects = order;

        // Preselect all non-Id fields available per object based on finalExportQueries
        const byObjFields = {};
        for (const q of this.finalExportQueries) {
            if (!byObjFields[q.objectName]) byObjFields[q.objectName] = new Set();
            for (const f of q.fields || []) byObjFields[q.objectName].add(f);
        }
        const optionsByObj = {};
        const valuesByObj = {};
        for (const obj of order) {
            const all = Array.from(byObjFields[obj] || []);
            const options = all.map(f => ({ label: f, value: f }));
            const defaults = all.filter(f => f !== 'Id');
            optionsByObj[obj] = options;
            valuesByObj[obj] = defaults;
            this.matchingConfigByObject.set(obj, defaults);
        }
        this.matchingFieldOptionsByObject = optionsByObj;
        this.selectedMatchingFieldsByObject = valuesByObj;

        // Reset compare/import state
        this.matchingCounts = {};
        this.matchingCountsByObject = new Map();
        this.matchedIdMap = new Map();
        this.finalIdRemap = new Map();
        this.importReport = [];

        // Open wizard
        this.matchingStep = 'select';
        this.showMatchingUIWizard = true;
    };

    // Wizard: change fields for a specific object
    handleMatchingFieldChangeForObject = (event) => {
        const obj = event.target && event.target.name;
        const val = event.detail && event.detail.value ? event.detail.value : [];
        if (!obj) return;
        this.selectedMatchingFieldsByObject = { ...this.selectedMatchingFieldsByObject, [obj]: val };
        this.matchingConfigByObject.set(obj, val);
    };

    // Wizard: proceed to run comparison
    handleMatchingCheck = async () => {
        this.error = undefined;
        this.isLoading = true;
        const countsObj = {};
        try {
            for (const objectName of this.matchingObjects) {
                const selected = this.matchingConfigByObject.get(objectName) || [];
                const fieldsToUse = selected.filter((f) => !!f);
                if (fieldsToUse.length === 0) {
                    this.matchingCountsByObject.set(objectName, { sourceCount: 0, destCount: 0, matchedCount: 0, unmatchedIds: [] });
                    countsObj[objectName] = { sourceCount: 0, destCount: 0, matchedCount: 0, unmatchedCount: 0 };
                    continue;
                }

                const sourceFields = ['Id', ...fieldsToUse.filter((f) => f !== 'Id')];
                const destFields = sourceFields;

                // Load source rows using generated Id IN clauses
                let sourceRows = [];
                for (const q of this.finalExportQueries.filter((def) => def.objectName === objectName)) {
                    const idList = this.extractIdInList(q.soql);
                    if (!idList || idList === 'NULL') continue;
                    const soql = `SELECT ${sourceFields.join(', ')} FROM ${objectName} WHERE Id IN (${idList})`;
                    const result = await this.runQueryWithSession(soql);
                ...
                // Use first selected field (prefer non-Id) to pre-filter destination rows
                const primaryField = fieldsToUse.find((f) => f !== 'Id') || fieldsToUse[0];
                const primaryValues = Array.from(
                    new Set(
                        sourceRows
                            .map((row) => row[primaryField])
                ...
                if (primaryValues.length) {
                    for (let idx = 0; idx < primaryValues.length; idx += 200) {
                        const batch = primaryValues.slice(idx, idx + 200);
                        if (!batch.length) continue;
                        const formattedValues = batch
                            .map((val) => `'${String(val).replace(/'/g, "\\'")}'`)
                            .join(',');
                        const whereField = primaryField === 'Id' ? 'Id' : primaryField;
                        const destSoql = `SELECT ${destFields.join(', ')} FROM ${objectName} WHERE ${whereField} IN (${formattedValues})`;
                        const destResult = await this.routeDestinationCall(
                            () => queryCurrent({ soql: destSoql }),
                            () => queryWithSession({ sessionId: this.destSessionId, instanceUrl: this.destInstanceUrl, soql: destSoql })
                        );
                        if (destResult && Array.isArray(destResult.rows)) {
                            destRows = destRows.concat(destResult.rows);
                        }
                    }
                }
                ...
                for (const row of destRows) {
                    const key = this.buildConcatenatedKey(row, fieldsToUse);
                    if (key) {
                        destKeyMap.set(key, row.Id);
                    }
                }

                let objMatchMap = this.matchedIdMap.get(objectName);
                if (!objMatchMap) {
                    objMatchMap = new Map();
                    this.matchedIdMap.set(objectName, objMatchMap);
                } else {
                    objMatchMap.clear();
                }
                let matchedCount = 0;
                ...
                for (const row of sourceRows) {
                    const key = this.buildConcatenatedKey(row, fieldsToUse);
                    const destId = key ? destKeyMap.get(key) : undefined;
                    if (destId && row && typeof row.Id === 'string') {
                        objMatchMap.set(row.Id, destId);
                        matchedCount += 1;
                    } else {
                ...
                this.matchingCountsByObject.set(objectName, { sourceCount, destCount, matchedCount, unmatchedIds });
                countsObj[objectName] = { sourceCount, destCount, matchedCount, unmatchedCount: unmatchedIds.length };
            }

            this.matchingCounts = countsObj;
            this.matchingStep = 'results';
        } catch (e) {
            this.error = e?.body?.message || e?.message || 'Matching failed';
        } finally {
            this.isLoading = false;
        }
    };

    // Extract Id IN (...) list from a SOQL string generated in finalExportQueries
    extractIdInList(soql) {
        // naive but safe for our generated queries format
        const m = (soql || '').match(/where\s+id\s+in\s*\((.+)\)/i);
        return m && m[1] ? m[1] : 'NULL';
    }

    get isMatchingStepSelect() { return this.showMatchingUIWizard && this.matchingStep === 'select'; }
    get isMatchingStepResults() { return this.showMatchingUIWizard && this.matchingStep === 'results'; }
    get isMatchingStepReport() { return this.showMatchingUIWizard && this.matchingStep === 'report'; }

    // Computed properties for template rendering (workaround for LWC limitations)
    get matchingRows() {
        return this.matchingObjects.map(obj => ({
            objectName: obj,
            options: this.matchingFieldOptionsByObject[obj] || [],
            values: this.selectedMatchingFieldsByObject[obj] || []
        }));
    }

    get matchingResultRows() {
        return this.matchingObjects.map(obj => ({
            objectName: obj,
            sourceCount: (this.matchingCounts[obj] || {}).sourceCount || 0,
            destCount: (this.matchingCounts[obj] || {}).destCount || 0,
            matchedCount: (this.matchingCounts[obj] || {}).matchedCount || 0,
            unmatchedCount: (this.matchingCounts[obj] || {}).unmatchedCount || 0
        }));
    }

    get hasMatchingUnmatchedRecords() {
        for (const obj of this.matchingObjects) {
            const counts = this.matchingCountsByObject.get(obj);
            if (counts && Array.isArray(counts.unmatchedIds) && counts.unmatchedIds.length) {
                return true;
            }
        }
        return false;
    ...
    get isMatchingImportDisabled() {
        ...
        return !this.hasMatchingUnmatchedRecords;
    }

    // Wizard: back to selection
    handleMatchingBack = () => {
        if (!this.showMatchingUIWizard) return;
        this.matchingStep = 'select';
    };

    // Wizard: cancel/close and clear state
    handleMatchingCancel = () => {
        this.showMatchingUIWizard = false;
        this.matchingStep = 'select';
        this.matchingConfigByObject = new Map();
        this.matchingCountsByObject = new Map();
        this.matchedIdMap = new Map();
        this.finalIdRemap = new Map();
        this.matchingObjects = [];
        this.matchingFieldOptionsByObject = {};
        this.selectedMatchingFieldsByObject = {};
        this.matchingCounts = {};
        this.importReport = [];
    };

    // Wizard: import non-matched
    handleMatchingImport = async () => {
        if (!this.isDestinationCurrentOrg) {
            this.error = 'Check & Import currently supports Destination = Use Current Org.';
            return;
        }
        this.error = undefined;
        this.isLoading = true;
        try {
            this.finalIdRemap = new Map();
            for (const obj of this.matchingObjects) {
                const matchedMap = this.matchedIdMap.get(obj) || new Map();
                this.finalIdRemap.set(obj, new Map(matchedMap));
            }

            const report = [];
            for (const objectName of this.matchingObjects) {
                const counts = this.matchingCountsByObject.get(objectName) || { unmatchedIds: [] };
                const pendingIds = Array.isArray(counts.unmatchedIds) ? [...counts.unmatchedIds] : [];
                if (!pendingIds.length) {
                    report.push({ objectName, successCount: 0, errorCount: 0, successes: [], errors: [] });
                    continue;
                }

                let sourceRows = [];
                while (pendingIds.length) {
                    const subset = pendingIds.splice(0, 200);
                    if (!subset.length) break;
                    const whereList = subset.map((id) => `'${id.replace(/'/g, "\\'")}'`).join(',');
                    const fields = new Set();
                    for (const q of this.finalExportQueries.filter((def) => def.objectName === objectName)) {
                        (q.fields || []).forEach((f) => fields.add(f));
                    }
                    fields.delete('Id');
                    const fieldList = Array.from(fields);
                    const soql = `SELECT Id${fieldList.length ? `, ${fieldList.join(', ')}` : ''} FROM ${objectName} WHERE Id IN (${whereList})`;
                    const result = await this.runQueryWithSession(soql);
                    if (result && Array.isArray(result.rows)) {
                        sourceRows = sourceRows.concat(result.rows);
                    }
                ...
                const refTargets = this.getRefTargetsByField(objectName);
                const outRecords = sourceRows.map((row) => {
                    const out = { Id: row.Id };
                    for (const key of Object.keys(row)) {
                        if (key === 'Id') continue;
                        let val = row[key];
                        const targetObj = refTargets.get(key);
                        if (targetObj && val) {
                            const mapForTarget = this.finalIdRemap.get(targetObj);
                            if (mapForTarget) {
                                if (typeof val === 'string' && mapForTarget.has(val)) {
                                    val = mapForTarget.get(val);
                                } else if (Array.isArray(val)) {
                                    val = val.map((v) =>
                                        typeof v === 'string' && mapForTarget.has(v) ? mapForTarget.get(v) : v
                                    );
                                }
                            }
                        }
                ...
                const insertResults = await insertRecordsCurrent({ objectName, records: outRecords });
                let successCount = 0;
                let errorCount = 0;
                const objectMap = this.finalIdRemap.get(objectName) || new Map();
                const successes = [];
                const errors = [];
                for (const r of insertResults || []) {
                    if (r && r.success && r.oldId && r.newId) {
                        objectMap.set(r.oldId, r.newId);
                        successCount += 1;
                        successes.push({ oldId: r.oldId, newId: r.newId });
                    } else {
                        errorCount += 1;
                        const oldId = r?.oldId || 'UNKNOWN';
                        const message = (r && r.errorMessage) ? r.errorMessage : 'Unknown insert error';
                        errors.push({ oldId, errorMessage: message });
                    }
                }
                this.finalIdRemap.set(objectName, objectMap);
                report.push({ objectName, successCount, errorCount, successes, errors });
            }

            this.importReport = report;
            this.matchingStep = 'report';
        } catch (e) {
            this.error = e?.body?.message || e?.message || 'Import failed';
        } finally {
            this.isLoading = false;
        }
    };

    // Wizard: save report (JSON only here; CSV can be added later)
    handleMatchingSave = () => {
        try {
            const payload = {
                startedAt: new Date().toISOString(),
                perObject: this.importReport
            };
            const dataStr = 'data:text/json;charset=utf-8,' + encodeURIComponent(JSON.stringify(payload, null, 2));
            const a = document.createElement('a');
            a.href = dataStr;
            a.download = 'check-import-report.json';
            a.click();
        } catch (e) {
            // no-op
        }
    ...
    // Helper: object order for matching is bottom-to-top of export order filtered by lastQueriedIdSets
    getRefTargetsByField(objectName) {
    ...
    async handleStartImport() {
        ...
        // Clear import-only output (do not mix with matching)
        this.importLogs = [];
        
        // Reset import status using the initialization method
        this.importStatus = this.getInitialImportStatus();
        this.importStatus.inProgress = true;
        this.importStatus.totalObjects = this.finalExportQueries.length;
        ...
        try {
            ...
            for (let i = this.finalExportQueries.length - 1; i >= 0; i -= 1) {
                ...
                const importingMsg = `Importing ${records.length} ${objectName} records...`;
                this.importStatus.detailedMessages.push(importingMsg);
                this.importLogs = [...this.importLogs, importingMsg];
                ...
                try {
                    ...
                    if (Array.isArray(insertResults)) {
                        let successCount = 0;
                        let errorCount = 0;
                        const successIdMap = new Map();
                        const errorMap = new Map();
                        
                        for (const r of insertResults) {
                            if (r && r.success && r.oldId && r.newId) {
                                mapForObject.set(r.oldId, r.newId);
                                successIdMap.set(r.oldId, r.newId);
                                successCount++;
                            } else {
                                errorCount++;
                                // Get the full error message from the result
                                const fullError = r.error?.message || r.error || r.errorMessage || JSON.stringify(r.errors) || 'Record insert failed';
                                errorMap.set(r.oldId, fullError);
                                const errMsg = `Error in ${objectName}: Record ${r.oldId} - ${fullError}`;
                                this.importStatus.detailedMessages.push(errMsg);
                                this.importLogs = [...this.importLogs, errMsg];
                            }
                        }
                        
                        this.importStatus.successCount += successCount;
                        this.importStatus.errorCount += errorCount;
                        
                        // Add to structured report
                        if (successCount > 0) {
                            this.importStatus.report.success.push({
                                object: objectName,
                                count: successCount,
                                idMap: Object.fromEntries(successIdMap)
                            });
                        }
                        
                        if (errorCount > 0) {
                            this.importStatus.report.error.push({
                                object: objectName,
                                count: errorCount,
                        ...
                        const doneMsg = `${objectName}: ${successCount} records imported successfully, ${errorCount} records failed`;
                        this.importStatus.detailedMessages.push(doneMsg);
                        this.importLogs = [...this.importLogs, doneMsg];
                    }
                } catch (objError) {
                    const errorMsg = objError.body?.message || objError.message || JSON.stringify(objError);
                    this.importStatus.errorCount += records.length;
                    
                    // Add batch error to report
                    this.importStatus.report.error.push({
                        object: objectName,
                        count: records.length,
                        errorMap: { batch: errorMsg }
                    });
                    
                    const batchErr = `Failed to import ${objectName}: ${errorMsg}`;
                    this.importStatus.detailedMessages.push(batchErr);
                    this.importLogs = [...this.importLogs, batchErr];
                }

                this.importStatus.processedObjects++;
            }

            if (this.importStatus.errorCount === 0) {
                this.importStatus.detailedMessages.push('Import completed successfully');
                this.importLogs = [...this.importLogs, 'Import completed successfully'];
            } else {
                // Create structured report string
                const reportStr = JSON.stringify({
                    success: this.importStatus.report.success,
                    error: this.importStatus.report.error
                }, null, 2);
                
                const endMsg = 'Import Report:\n' + reportStr;
                this.importStatus.detailedMessages.push(endMsg);
                this.importLogs = [...this.importLogs, endMsg];
                this.error = `Import completed with ${this.importStatus.successCount} successes and ${this.importStatus.errorCount} failures. Check detailed report below.`;
            }
        } catch (e) {
            const msg = e.body?.message || e.message || 'Import failed';
            this.error = msg;
            this.importStatus.detailedMessages.push(`Import failed: ${msg}`);
    ...
    // Clear downstream state when earlier inputs change
    clearAfter(scope) {
        // Always stop matching flow and clear its outputs when upstream changes
        this.showMatchingUI = false;
        this.currentMatchingObject = undefined;
        this.matchingQueryList = [];
        this.matchingFieldsOptions = [];
        this.selectedMatchingFields = [];
        this.matchingLogs = [];

        // If plan or selection inputs changed, clear plan/export/final queries/import state
        const resetPlan = ['object', 'depth', 'excludedObjects'].includes(scope);
        const resetExport = resetPlan || ['exportLimit'].includes(scope);
        const resetFinal = resetExport || ['soql', 'destCredentials'].includes(scope);
        ...
        if (resetFinal) {
            ...
            // also clear import outputs
            this.importStatus = this.getInitialImportStatus();
            this.importLogs = [];
        }
    }

    // Simple utility to build concatenated key of selected fields
    buildConcatenatedKey(record, fields) {
        try {
            const list = Array.isArray(fields) ? fields : [];
            return list.map((f) => {
                const v = record && f in record ? record[f] : undefined;
                if (v === null || v === undefined) return '';
                if (typeof v === 'object') {
                    // If relationship object, prefer Id; otherwise stringify
                    return (v && typeof v.Id === 'string') ? v.Id : JSON.stringify(v);
                }
                return String(v);
            }).join('|');
        } catch (e) {
    ...
    // Advance through reversed finalExportQueries, presenting UI for each object
    displayNextMatchingCheck() {
        if (!this.matchingQueryList || this.matchingQueryList.length === 0) {
            // Done
            this.showMatchingUI = false;
            this.currentMatchingObject = undefined;
            const total = this.idRemap ? this.idRemap.size : 0;
            this.matchingLogs = [
                ...this.matchingLogs,
                `Matching complete. Total Source->Destination mappings: ${total}`
            ];
            return;
        }

        const queryDef = this.matchingQueryList.pop(); // {objectName, soql, fields, count}
        this.currentMatchingObject = queryDef;
        const fields = Array.isArray(queryDef?.fields) ? queryDef.fields : [];
        this.matchingFieldsOptions = fields.map((f) => ({ label: f, value: f }));
        // Default to all fields (except avoid duplicates)
        const uniq = Array.from(new Set(fields));
        this.selectedMatchingFields = [...uniq];
        this.showMatchingUI = true;
    }

    handleMatchingFieldChange = (event) => {
        this.selectedMatchingFields = event.detail?.value || [];
    };

    // Execute matching for current object and proceed to next
    async handleRunMatchingCheck() {
        if (!this.currentMatchingObject || !this.currentMatchingObject.objectName) {
            this.error = 'Nothing to match. No current object.';
            return;
        }
        const matchFields = (this.selectedMatchingFields || []).filter((f) => !!f);
        if (matchFields.length === 0) {
            this.error = 'Select at least one field to match on.';
            return;
        }
        ...
        try {
            // 2) Reverse direction per requirement:
            // Take the SOQL from the DESTINATION side (opposite of earlier approach).
            // We query DESTINATION with the existing final SOQL (which was built from source Ids),
            // and query SOURCE with a broad SELECT on the same fields for key construction.
            const destSoql = this.currentMatchingObject.soql; // use stored batched IN (...) on destination
            const fieldsToQuery = ['Id', ...matchFields.filter((f) => f !== 'Id')];
            const sourceSoql = `SELECT ${fieldsToQuery.join(', ')} FROM ${this.currentMatchingObject.objectName}`;

            // Query destination (current org or external dest session) using stored batch SOQL
            const destResult = await this.routeDestinationCall(
                () => queryCurrent({ soql: destSoql }),
                () => queryWithSession({ sessionId: this.destSessionId, instanceUrl: this.destInstanceUrl, soql: destSoql })
            );

            // Query source rows broadly for the selected fields
            const sourceResult = await this.runQueryWithSession(sourceSoql);

            const destRows = (destResult && Array.isArray(destResult.rows)) ? destResult.rows : [];
            const sourceRows = (sourceResult && Array.isArray(sourceResult.rows)) ? sourceResult.rows : [];

            // Build SOURCE key -> Id map (reverse of previous direction)
            const sourceKeyMap = new Map();
            for (const row of sourceRows) {
                const key = this.buildConcatenatedKey(row, matchFields);
                if (key) sourceKeyMap.set(key, row.Id);
            }

            // For each destination row, find source by key and map SourceId -> DestinationId
            let matched = 0;
            for (const row of destRows) {
                const key = this.buildConcatenatedKey(row, matchFields);
                const srcId = key ? sourceKeyMap.get(key) : undefined;
                if (srcId && row && typeof row.Id === 'string') {
                    this.idRemap.set(srcId, row.Id);
                    matched += 1;
                }
            }

            // Log progress to matching-only output
            this.matchingLogs = [
                ...this.matchingLogs,
                `Matched ${matched} records for ${this.currentMatchingObject.objectName} using fields: ${matchFields.join(', ')}`
            ];
